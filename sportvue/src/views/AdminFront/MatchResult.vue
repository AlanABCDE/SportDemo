<template>
  ...
  <el-table-column fixed="right" label="操作" width="120">
    <template v-slot="scope">
      <el-button link type="primary" size="small" @click="MatchDetail(scope.row.matchId)">现在上分</el-button>
    </template>
  </el-table-column>
</el-table>
  ...
  <el-dialog v-model="dialogVisible" title="开始上分">
    <el-form :inline="false" :label-position="labelPosition" v-model="formData" >
      ...
      <el-form-item label="比赛状态" prop="matchStatus">
        <el-col :span="16">
          <el-select v-model="formData.matchStatus" placeholder="选择一个状态">
      ...
      <el-form-item label="比赛得分">
        <el-input type="textarea" :autosize="{ minRows: 2, maxRows: 4 }" placeholder="请输入内容" v-model="formData.matchScore">
        </el-input>
      </el-form-item>
      <el-form-item label="比赛选手">
        <el-input type="textarea" :autosize="{ minRows: 2, maxRows: 4 }" placeholder="请输入内容" v-model="formData.matchPlayer">
        </el-input>
      </el-form-item>
      <el-form-item label="比赛排名">
        <el-input type="textarea" :autosize="{ minRows: 2, maxRows: 4 }" placeholder="请输入内容" v-model="formData.matchRank">
        </el-input>
      </el-form-item>
    </el-form>
    <template #footer>
...
export default {
  ...
  methods:{
    ...
    submit(){
      ...
      axios.post("http://localhost:9090/match/updateResult", this.formData).then(res => {
